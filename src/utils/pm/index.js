...
function exists(name, cb) {
  const exitServer = REG.find((item) => {
    return item.name === name;
  });
  if (exitServer) {
    console.log(`${name}服务${exitServer.child ? '' : '不'}存在`);
    if (cb) cb(!!exitServer.child);
    return !!exitServer.child;
  }
  if (cb) cb(false);
  return false;
}
...
function start(pi, cb) {
  ...
  if (!pi.script && !pi.cwd)
    return cb(`Cannot start process without 'script' or 'cwd'`);
  ...
  get_script_1(pi, (script) => {
    ...
    const handler = getScriptHandler(script);
  ...
  function fixAsarIssue(pi) {
    if (pi.cwd.includes('/app.asar/') || pi.cwd.includes('\\app.asar\\')) {
      const p = pi.cwd.split('app.asar');
      if (p.length == 2) {
      ...
      const p = pi._script.split('app.asar');
      if (p.length == 2) {
  ...
  function get_script_1(pi, cb) {
    ...
    const pkg = path.join(pi.cwd, 'package.json');
...
function startAgain(pi) {
  const handler = getScriptHandler(pi._script);
...
function restartIfNeeded(pi) {
  ...
  const intv = get_restart_interval_1();
  ...
  function get_restart_interval_1() {
    ...
    if (pi.restartAtNdx >= pi.restartAt.length)
      pi.restartAtNdx = pi.restartAt.length - 1;
